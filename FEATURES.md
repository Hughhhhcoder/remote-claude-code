...
**batch 32** (v0.2 arc) · Observability + dev perf HUD:
- B32-B Session timeline view — ✅ 完成 (2026-05-10)。新增 `packages/web/src/chat/SessionTimeline.tsx`(282 LOC · 新)只读竖向时间线,合并两源事件:(1) 当前会话 chat messages(通过 ChatHeader → ChatPane → ChatSurface 已在内存的 `stream.messages()`)每条消息一个节点(role 标签「你/助手/系统」+ 140 char 预览从首个 text/thinking/code/diff 段截取),assistant 含 `tool_use` 段时额外追加「工具调用 · {tool}」子节点(ts+1 保序),便于看到模型调工具的时刻;(2) 审计条目走既有 `audit.query.request → audit.entries` 一次性往返(limit 500, 3s 超时,与 `BugReportModal.fetchAudit` 同模式),按 `details.sid === props.sid || details.sourceSid === props.sid || details.id === props.sid` 过滤(session.new 用 sid / session.fork 用 sourceSid / share.create 用 id · 覆盖 host 侧审计写入点的三种字段形态)。节点按 ts 升序排序渲染:左侧 `absolute` 竖线轨道 + 圆点(`rcc-` tone bg),右侧 HH:MM:SS + label + 可选 audit kind 徽章 + 140 char detail。tone 映射:`user→accent`、`assistant→text-primary`、`system→text-muted`、`tool→warning`、`audit→success`,全部语义 token,零 hardcoded 颜色。`auditLabel()` 把常见 kind 映射到中文友好标签(`session.new`=会话创建 / `session.close`=会话关闭 / `share.create`=创建分享 / `approval.granted`=批准工具调用 等),未知 kind 回落原 kind 字符串保持可读。loading/empty/error 三态完整:审计未达「加载审计日志…」、达后 0 事件「暂无事件」、3s 超时显示 danger 色错误文本但不阻塞消息节点渲染。`packages/web/src/chat/ChatHeader.tsx`(241→280 LOC · +39)**单按钮** 加法(严格遵守「minimal edit, do not break export menu wiring」):顶部新 import `type { RccClient } from "../client"` + `Dialog` + `SessionTimeline`;props 加可选 `client?: RccClient`;组件内加 `const [timelineOpen, setTimelineOpen] = createSignal(false);` 紧邻既有 `exportOpen`;右簇在 Export 按钮**之前**插 `<Show when={props.client}>` 包裹的 🕐 IconButton(`aria-label="会话时间线"`)仅在有 client 时挂,未破坏 Popover anchor ref / B28-A export menu dispatch / B30-A 其它线;文件末 Popover 之后挂 `<Show when={props.client}><Dialog open={timelineOpen()} …><SessionTimeline client={…} sid={sid()} messages={props.messages ?? []}/></Dialog></Show>`,复用既有 `Dialog` primitive(mobile <640px 自动 bottom-sheet + drag handle + `env(safe-area-inset-bottom)`、desktop ≥640 centered md size;focus trap / ESC / backdrop / scroll-lock 全走 primitive 内置行为)。`packages/web/src/chat/ChatPane.tsx`(167→169 LOC · +2)prop 面加 `client?: RccClient` 并透传到 `<ChatHeader client={props.client}/>`;`packages/web/src/chat/ChatSurface.tsx`(451→452 LOC · +1)把 `client={props.client}` 加到 ChatPane 调用。**零协议改动**;既有 `audit.query.request/audit.entries` frame(B30-C 落地)直接用;未新增 store、未触碰 MessageList / MessageRow / Composer / SlashPalette / MentionPopover / streaming.ts / ContextInjector / exportChat / 任何 `blocks/*`。移动验证:375px Dialog 自动切 bottom-sheet,时间线 `pl-5` 左轨道 + `pb-4` 节点间距在 iOS safari 单手拇指可滚,HH:MM:SS `tabular-nums` + `font-mono` 对齐保持不抖,tool 调用「工具调用 · {tool}」label 配 input 140 char 预览避免长字串横撑。`pnpm -F @rcc/web typecheck ✅`(零错零警)。LOC:SessionTimeline.tsx 282(新)、ChatHeader.tsx 280(+39)、ChatPane.tsx 169(+2)、ChatSurface.tsx 452(+1)。
- B32-C Client-side render trace + perf overlay — ✅ 完成 (2026-05-10)。新增 `packages/web/src/dev/PerfOverlay.tsx`(218 LOC · 新)dev-only HUD:固定 `bottom-right` 圆角胶囊,`z-50`,`font-mono text-[11px]`,半透明暗底 `rgba(17,17,20,0.82)` + `backdrop-blur-sm` + 细边框,显示 `FPS · last-frame ms · WS msg/s · heap MB`,FPS <45 切 `#f87171` 红字提示。渲染循环走 `requestAnimationFrame(loop)`,每帧更新 `frameMs`(`Math.round(dt*10)/10` 一位小数),FPS 按 500ms 窗口累计 `frames*1000/fpsAccum` 求均值(降低抖动 vs 每帧即时值);WS msg/s 走 1s rolling window,读全局 `window.__rccPerfWSCount` 差值换算(`Math.round((cur-base)*1000/elapsed)`);heap 走 `performance.memory.usedJSHeapSize/1MiB`(Chromium-only,未暴露时隐藏该段保持简洁)。`installWSCounter()` **一次性** monkey-patch `WebSocket.prototype.addEventListener`:当 type==='message' 且 listener 非空,用 wrapper 包裹先 `window.__rccPerfWSCount++` 再转发到原 listener(支持 function + EventListenerObject 两种签名);`window.__rccPerfWSWrapped` 守卫幂等。刻意选 patch 路径而非改 `client.ts`,以严格遵守「no protocol/host changes + 不触碰 non-owned 文件」约束;counter 通过 `window` 全局 hop 在 overlay 与 client 之间解耦,零 import cycle。Toggle 两路:(1) `?debug=1` 自动挂载(main.tsx 开机一次性读 `URLSearchParams`);(2) 运行期 `Cmd/Ctrl+Shift+P` 切换 `visible` signal(`onKey` 全局 `keydown` listener,`preventDefault` 吞 Chrome 的系统「Print」默认行为)。Drag-to-reposition:`onPointerDown` 捕获 pointer + 快照基线(`px,py,ox,oy`),`onPointerMove` 算 `dx/dy` 反向应用到 `right/bottom` offset(胶囊锚点在右下所以拖拽方向取反),`Math.max(0, …)` clamp 防出屏;`onPointerUp` 释放 capture 并把 `{x,y}` 写进 `sessionStorage["rcc:perf-overlay:pos"]`(仅 tab 内持久,关标签归位默认 12,12)。`packages/web/src/main.tsx`(22→37 LOC · +15)在 `render(App)` 之后、`installSW` 之前补一个迷你 mount 点:`if URLSearchParams.get('debug')==='1'` 动态 `createElement('div')` 挂 `id="rcc-perf-overlay-root"` 到 `document.body`,再 `render(() => <PerfOverlay/>, perfRoot)`。overlay mount 不共享主 App 的 `root` 节点(避免任何意外的 layout 影响);在 App 之外独立 render,确保 App 的 `<ErrorBoundary>` 崩溃时 HUD 仍能显示「最后一帧 ms / 当前 fps」帮助诊断。约束对齐:**零 host/protocol 改动**;未触碰 `MetricsPanel.tsx`(B32-A)/ `chat/ChatHeader.tsx`(B32-B)/ `chat/*` / `blocks/*` / `SettingsModal.tsx` / `App.tsx`;main.tsx 编辑量 +15 行(新 import + `if(debug) mount` 段落)已是最小;生产包若未带 `?debug=1` query 则永不挂载 overlay,运行时开销仅 1 次 URL 解析。HUD 显示样例:`60 fps · 16.7ms · 3 msg/s · 124MB`。快捷键:`Cmd/Ctrl+Shift+P` toggle。持久化:`sessionStorage` key `rcc:perf-overlay:pos` (draggable 后的 offset)。`pnpm -F @rcc/web typecheck ✅`(零错零警)。LOC:PerfOverlay.tsx 218(新)、main.tsx 37(+15)。
...
**batch 31** (v0.2 arc) · Crash + health UX:
- B31-C Sensitive-redacted log export (host-side) — ✅ 完成 (2026-05-10)。新增 `packages/host/src/logs-export.ts`(138 LOC · 新)导出 `buildLogsExport(): Promise<LogsExportBundle>`:并发 `tailLines(auditLogPath, 1000)` + `tailLines(crashLogPath, 500)` + `versionSummary()` + `readFile(configPath())`,返回统一 JSON 文档 `{v:1, exportedAt, version:{version,buildTime,node}, audit:{path,lineCount,entries[]}, crashes:{path,lineCount,lines[]}, config:{path,redacted}}`,无 tarball 依赖。`redact(value, seen=WeakSet)` 递归遍历任意 JSON 值,key 匹配 `/token|password|secret|key|credentials|cert|vapid/i` 则值替换为 `"[REDACTED]"`,Array/Object/primitive/null 全覆盖,`WeakSet` 防 cycle 回归 `"[Circular]"`。`tailLines` 读全文 split("\n") 去掉 split 产生的末尾空 element 再 slice 尾 N 行(`total` 报原始行数,便于判断是否被截断);`ENOENT` 返回 `{lines:[], total:0}` 不报错(全新安装 audit.jsonl / crashes.log 尚不存在的情况)。审计条目 JSON.parse,失败的行保留为 `{__unparsed: line}` 便于 bug 分析;config 整体不可解析时也给 `{__unparsed:"…"}` 占位。`packages/host/src/index.ts`(4591→4626 LOC · +35)在 `/api/v1/*` REST 分派前插 `GET /api/v1/logs/export` 分支:`authenticate(req)` 失败 401(`x-rcc-auth-reason` 一致),成功调 `buildLogsExport()` 后 `audit.write({kind:"logs.export", deviceId, ip, details:{auditLines,crashLines}})` 留痕(self-referential 审计,用户知晓谁导出了什么),response `content-type: application/json;charset=utf-8` + `cache-control: no-store` + `JSON.stringify(bundle)`;置于 REST 分派前是为了不被 rest.ts 的 `writeError(404,"no_route")` 兜底吃掉(rest.ts 已有 `authenticate` 层,但路由表是白名单式的)。顶部 `import { buildLogsExport } from "./logs-export.ts"` 紧邻 `version.ts` 导入。`packages/web/src/SettingsModal.tsx`(652→728 LOC · +76)新增 `LogsExportSection`(置于 `KeysSection` 后 `AboutSection` 前,始终显示,独立行):标题「日志导出」/ 正文「导出诊断日志」+ hint `打包最近 1000 条审计 + 500 行崩溃日志 + 版本与脱敏后的 config,令牌/密钥/凭证字段自动替换为 [REDACTED]`,右侧 `📥 导出日志` 按钮(busy 时「导出中…」+ disabled)。点击 `fetch("/api/v1/logs/export", {headers:{authorization:"Bearer "+loadToken()}})`,成功 `resp.blob()` 建 object URL,`<a download="rcc-logs-${isoTs}.json">` 注 DOM 点击触发下载(Safari 兼容 1s 延迟 revokeObjectURL),失败展示 `text-rose-400` 行内错误。`loadToken()` 从既有 `./auth.ts` 导入,未改 auth.ts 表面。约束对齐:未触碰 `ErrorBoundary.tsx` / `BugReportModal`(B31-B 域)/ `chat/*` / `blocks/*` / 任何 `hooks/*`;redaction 规则完全在 host 侧执行(web 不缓存、不转发 config 内容);自审计 audit 条目让 `logs.export` 本身也出现在下次导出里,形成可追溯链。`pnpm -r typecheck ✅`(4 包零错零警)。Endpoint: `GET /api/v1/logs/export`。Response shape 样例:`{"v":1,"exportedAt":1747000000000,"version":{"version":"0.1.15","buildTime":1747000000000,"node":"v25.0.0"},"audit":{"path":"/Users/u/.rcc/audit.jsonl","lineCount":42,"entries":[{"ts":…,"kind":"auth.pair","deviceId":"…","details":{…}}, …]},"crashes":{"path":"/Users/u/.rcc/crashes.log","lineCount":0,"lines":[]},"config":{"path":"/Users/u/.rcc/config.json","redacted":{"tunnel":{"mode":"named","credentialsFile":"[REDACTED]"},"vapid":"[REDACTED]"}}}`。LOC:logs-export.ts 138(新)、index.ts +35、SettingsModal.tsx +76。
- B31-A Crash page redesign + client crash log sink — ✅ 完成 (2026-05-10)。`packages/web/src/primitives/ErrorBoundary.tsx`(156→191 LOC,+35)重写 fallback:居中大号 `🔶` Claude diamond(`text-accent text-6xl/7xl`)+ `font-serif` 28/32px "出现了一个错误" 标题 + 可选 `范围:{scope}` 副文案,错误块改为点击展开式 collapse（button 默认关闭,展开后显示 `name: message` 红字 + 栈 `<pre>` 最大高 240px 可滚动）,按钮组桌面端右对齐横排、`<640px` 垂直栈布,顺序为 `复制错误信息` / `重置`(调 Solid reset 重试渲染)/ `刷新页面`(primary · `bg-accent text-white` · `window.location.reload`);容器改用 safe-area inline style(`max(env(safe-area-inset-*), 16px)`)顶替原 `p-4`,覆盖 iOS 刘海与横屏圆角;复制格式按任务规格 `{scope}\n{name}: {message}\n{stack}\n{userAgent}\n{timestamp}` 纯文本(不再 JSON)。导出 `buildCrashReport` / `CrashReport` 类型供 App wire 复用。所有 token 合规(`bg-bg-page` / `text-text-primary` / `text-text-muted` / `text-danger` / `text-accent` / `bg-bg-surface` / `border-border-subtle`),无 zinc。`packages/protocol/src/index.ts`(2770→2784 LOC,+14)新增 `ClientCrashReport { v, t:"client.crash.report", scope, stack, ua, ts }`(所有字段 string/number,无嵌套 schema 降低解析失败面),加入 Frame union 紧随 `HealthWarn`。新增 `packages/host/src/crashes.ts`(52 LOC · 新):`appendClientCrash({scope,stack,ua,ts})` 复用 `crash.ts` 导出的 `crashLogPath`(`~/.rcc/crashes.log`),写 JSONL 行 `{time,type:"client",scope,message(栈首行),stack,ua}`(`type:"client"` 与 host 的 `uncaughtException`/`unhandledRejection` 区分,便于 grep);永不抛异常,失败只 `console.error`。`packages/host/src/index.ts`(4576→4591 LOC,+15)补 `case "client.crash.report"` 分支:`metrics.incr("crashes")` + `void appendClientCrash(…)` 后 return,无 ack、无 broadcast,纯 fire-and-forget 日志通道。`packages/web/src/App.tsx`(661→686 LOC,+25)新增 `reportCrash(scope, err)` helper 把 Error 序列化为帧字段发 `client.send`(ws 关闭吞掉异常不 mask 原错误);两处 `<ErrorBoundary scope="app">` / `scope="chat">` 各挂 `onError={(err) => reportCrash(scope, err)}`,走既有 `SolidErrorBoundary` fallback 的 onError 桩。约束对齐:未触碰 `WorkflowRunPanel` / `MobileTabRouter` / `MobileSettingsPane` / `Composer` / `MessageList` / `MessageRow` / 任何 `blocks/*` / `SettingsModal`;375px 移动端布局:按钮组 `flex-col sm:flex-row`,safe-area 四向 inline style,标题 28→32px 响应式,details pre 水平/垂直滚动 + `break-all`。Sample payload: `{"v":1,"t":"client.crash.report","scope":"chat","stack":"TypeError: Cannot read properties of undefined (reading 'foo')\n    at MessageList (…)\n    at …","ua":"Mozilla/5.0 (iPhone; CPU iPhone OS 17_0 …) Safari/605.1.15","ts":1746892800123}`,落盘 `{"time":1746892800123,"type":"client","scope":"chat","message":"TypeError: Cannot read properties of undefined (reading 'foo')","stack":"TypeError: …\n    at …","ua":"Mozilla/5.0 …"}`。`pnpm -r typecheck ✅`(4 包零错零警)。
...
**batch 33**:
- B33-A Metrics 面板重设计(sparkline + 对齐 Claude 风)— DONE (batch 32/A) · cards grid + reusable `<SparklineSVG>` primitive, token-based (accent/success/warn/danger), Charter serif titles, mobile stacks <640, client-side approval latency p50/p95, active-sessions + crash series; no protocol changes. Files: `packages/web/src/MetricsPanel.tsx`, `packages/web/src/primitives/Sparkline.tsx`.
- B33-A-cleanup Final cleanup: remove legacy `ChatView.tsx` + `useIsMobile.ts` — ✅ 完成 (batch 33 · 2026-05-10)。清掉 Phase 5/6 遗留的双路径代码:`packages/web/src/ChatView.tsx`(619 LOC · 旧 chat renderer,仅 `SharedReadonlyView.tsx` 在用)与 `packages/web/src/useIsMobile.ts`(36 LOC · 旧 767px 媒体查询 hook)整体删除,`src/` 从 2 个冗余文件缩到 0。`packages/web/src/chat/ChatSurface.tsx` 契约新增可选 `readOnly?: boolean`(最小变更 + 新属性 13 LOC):`createMemo` 的 shared-text 初始化在 `props.readOnly` 下 early return(跳过 CRDT `input-draft` 订阅),JSX 的 `composerSlot` 在 readOnly 时传 `<></>` 空 fragment(绕过 ChatPane 的 `PlaceholderComposer` fallback),`voiceError` / `injectOpen` 两个 `<Show>` 本身由 Composer 内按钮驱动,composer 不渲染自然不会触发。`packages/web/src/SharedReadonlyView.tsx`(179→188 LOC)把 `<ChatView client={client} sid={sess().id} />` 换成 `<ChatSurface client={client} sid={sess().id} session={sess()} sessions={sessions()} commands={[]} readOnly />`,`commands={[]}` 让内部 `toSlashCommands` 得到空数组(readonly 下也不会打开 palette,纯 defensive)。四个 `useIsMobile` 老路径 import 迁到 `hooks/useMediaQuery`:`CommandPalette.tsx`(`./useIsMobile.ts` → `./hooks/useMediaQuery.ts`)、`PermissionApproval.tsx`(同)、`shell/ShortcutHelp.tsx`(`../useIsMobile.ts` → `../hooks/useMediaQuery.ts`)、`chat/blocks/DiffBlock.tsx`(`../../useIsMobile` → `../../hooks/useMediaQuery`,任务范围外但必须同步否则 build 坏)。`hooks/useMediaQuery.ts` doc-comment 移除对已删文件 `../useIsMobile.ts` 的指涉(`useIsMobile` 使用 `(max-width: 639px)`,旧版 `767px`—调用方视觉阈值微调但既有 callers 原本就分散在 640/767 两组,统一到 639 符合其余 Phase 11+ 代码的断点体系)。`App.tsx` 的 `SharedReadonlyView` 挂载路径(`readShareTokenFromLocation()` → `/share?share=<token>`)未动,`App.tsx:119` 的 `<SharedReadonlyView shareToken={shareToken} />` 保持不变,只读守护(`status==="unauthorized"` / `status==="connecting"` / 无 session)原始路径完整。`grep -rn "useIsMobile\|ChatView" packages/web/src` 最终:所有 `useIsMobile` imports 只剩 `hooks/useMediaQuery` 一个源,`ChatView` 仅残留 3 条无害注释(`streaming.ts:48/52/369` 注明 "ChatView.tsx:52 行为" 等历史参考,`ChatSurface.tsx` doc comment "for the old ChatView")— 纯文档性,不编译,不影响运行时。`pnpm -F @rcc/web typecheck ✅`(零错零警)。LOC: ChatView.tsx -619 / useIsMobile.ts -36 / ChatSurface.tsx +13 / SharedReadonlyView.tsx +9 / 其余 4 文件仅改 import 行。净清理 -633 LOC。
...
- B35-C 嵌入式 tour(首次启动引导)— ✅ 完成 (batch 34/A · 2026-05-10)。首次配对 + 零会话时挂载 3 步 spotlight 引导:(1) sidebar 新建会话按钮、(2) composer 输入框、(3) 移动端导航(仅 compact 视口)。SVG mask 把暗色 overlay 裁出圆角空洞,accent 描边高亮;卡片桌面浮在目标右/左/下(视口自适应),移动端固定在底部(尊重 `env(safe-area-inset-bottom)`)。右上 skip 按钮 + 卡片内 Skip link 双重逃生,Esc / Space / Enter 键盘支持,完成或跳过后写 `localStorage["rcc.onboardingTour.completed"]=1`,下次不再展示。Target 元素通过 `[data-tour-target="…"]` 属性查找,`ResizeObserver` / scroll / resize 监听实时重算 rect。语义 token 全用 `bg-bg-surface` / `text-text-primary` / `border-border-subtle` / `bg-accent` — 无 zinc。Files: `packages/web/src/OnboardingTour.tsx` (+329 LOC new)、`packages/web/src/App.tsx` (+2:import + mount `<OnboardingTour active={sessionsStore.sessions().length === 0} />` after all modals)、`packages/web/src/shell/Sidebar.tsx` (+1:`data-tour-target="new-session"` on 新建会话 Button)、`packages/web/src/chat/Composer.tsx` (+1:`data-tour-target="composer"` on textarea)、`packages/web/src/i18n/zh.ts` + `en.ts` (+11 each: tour.* 文案 8 键 × 2 语言 + aria/skip/next/done)。`pnpm -F @rcc/web typecheck ✅`。
- v0.2 batch 34/C · Accessibility + PWA + HTML meta polish — ✅ 完成 (2026-05-10)。纯增量补丁,无任何逻辑改动。(1) A11y:扫描了所有 `IconButton` 调用点(13 文件)+ shell/primitives 的 close/X 按钮,绝大多数 v0.2 canonical 组件(TopBar / Composer / Dialog / Toast / ShortcutHelp / SettingsPane / InboxItem / NotebookEntry / ChatHeader / MessageRow / FileBrowser / FilePreview / RecordingPanel / RecordingPlayback / PeersPane / NotebookPane)已在前批次配齐 `aria-label`;为三个 legacy tab 的错误横幅关闭 ✕ 按钮补上缺失的 `aria-label="关闭错误提示"` + `type="button"`,files: `packages/web/src/McpTab.tsx:148`、`PermissionsTab.tsx:189`、`HooksTab.tsx:292` (+3 `<button>` 各 +2 属性)。未动 App.tsx / Sidebar.tsx / OnboardingTour.tsx(parallel B34-A 拥有)。(2) PWA manifest polish:`packages/web/public/manifest.webmanifest` 把 `theme_color` 与 `background_color` 从 `#09090b` (旧 zinc-950) 换成 `#1a1816` (tokens.css `--bg-page: 26 24 22`,即当前 dark mode page 背景),首次启动 PWA splash 与 iOS 状态栏 tint 才匹配 Claude 暖色基调;已验证 `display:"standalone"`/`display_override:["standalone","minimal-ui"]`/`orientation:"any"` 仍在;`categories` 重排为 `["productivity","developer","utilities"]`(productivity 置首,按任务指定);`shortcuts`(B29-C 添加)与 `share_target` 保留。(3) HTML meta polish:`packages/web/index.html` `theme-color` 同步到 `#1a1816`;新增 Open Graph(`og:title`/`og:description`/`og:type=website`/`og:site_name=rcc`)+ Twitter card(`twitter:card=summary`/`twitter:title`/`twitter:description`)共 7 条 meta,站点被分享到 IM / 社交时可正确展示 preview;保留 `viewport-fit=cover`、apple-mobile-web-app-*、mobile-web-app-capable 全套 PWA 头部。`pnpm -F @rcc/web typecheck ✅`(零错零警)。LOC: manifest.webmanifest 改 3 行 / index.html +7 行 / McpTab.tsx +4 / PermissionsTab.tsx +4 / HooksTab.tsx +4。共 4 aria-label 增量(1 existing `关闭` 模式复用 + 3 new `关闭错误提示`)+ 2 manifest 键调色 + 7 HTML meta 标签。
