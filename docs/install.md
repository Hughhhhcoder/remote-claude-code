...
```sh
curl -fsSL https://raw.githubusercontent.com/example/rcc/main/scripts/install.sh | bash
```
...
| `RCC_REPO` | `example/rcc` | GitHub `<owner>/<repo>`(fork 用户可改) |

示例:

```sh
RCC_VERSION=v0.1.0 curl -fsSL https://raw.githubusercontent.com/example/rcc/main/scripts/install.sh | bash
```

## 方式 2 · Homebrew (macOS / Linuxbrew)

```sh
brew tap example/rcc
brew install rcc
```
...
```sh
brew uninstall rcc
brew untap example/rcc
```

## 方式 3 · 手动下载 tar.gz

前往 [GitHub Releases](https://github.com/example/rcc/releases/latest),挑对应平台:
...
```sh
curl -fsSLO https://github.com/example/rcc/releases/download/v0.1.0/rcc-0.1.0-darwin-arm64.tar.gz
curl -fsSLO https://github.com/example/rcc/releases/download/v0.1.0/rcc-0.1.0-darwin-arm64.tar.gz.sha256
...
```sh
git clone https://github.com/example/rcc.git
cd rcc
